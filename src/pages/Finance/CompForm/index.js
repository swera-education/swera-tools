...
import CompInterestBlog from './CompInterestBlog';
...
export default class Finance extends Component {
  ...
  dataFun = ({initialBalance, tenure, compoundInterval, interestRate, regularDeposit,
    ...
    for (let i = 0; i <= tenure; i++) {
        ...
        let tableYearData = {};
        if (i > 0) {
          let prevFutureValue = lastFutureValue;
          lastFutureValue = lastFutureValue * (Math.pow((1 + interestRate / compoundInterval), compoundInterval));
          let s2 = 0;
          s2 = regChangingDeposit * ((Math.pow((1 + interestRate / regularInterval), regularInterval) - 1) / (interestRate / (regularInterval)))
          if (isBeginning == true)
            s2 = s2 * (1 + interestRate / regularInterval)
          lastFutureValue =  lastFutureValue + s2;
          totalDep = parseInt(totalDep, 10) + regChangingDeposit * regularInterval;
          interest = lastFutureValue - prevFutureValue - (regChangingDeposit * regularInterval);
        ...
        totalInterest = lastFutureValue - totalDep;
  ...
  submitForm = values => {
    ...
    const interestRate = parseInt(values.interestInterval, 10) * parseInt(values.interestRate, 10)/100;
    const compoundInterval = parseInt(values.compoundInterval, 10);
    ...
    const dataFunResult  = this.dataFun({initialBalance, tenure, compoundInterval,
    ...
    setTimeout(() => {
      document.getElementById('compoundResult').scrollIntoView({behavior: 'smooth', block: "end"})
    }, 100);
    return null;
  }
      ...
      <div className="container-fluid">
        <div className="row">
          <div className="col-md-8">
            <CompForm onSubmit={this.submitForm} />
          </div>
        </div>
        <div className = "row" id="compoundResult">
          <div className="col-md-8" >
